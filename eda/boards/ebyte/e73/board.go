...
var (
	...
	mountHoles = &eda.Component{
		Components: eda.Components{
			boards.MountHole.Arrange(transform.Move(-5, -8)),
			boards.MountHole.Arrange(transform.Move(5, -8)),
		},
	}
	...
	Board_nRF52840 = &eda.Component{
		...
		Components: eda.Components{
			...
			boards.Logo.Arrange(transform.Move(-6.3, -6)),

			eda.CenteredText("E73").Arrange(transform.Scale(1.2, 2).Move(-2.5, -6.8)),

			eda.CenteredText("-2G4M08S1C").Arrange(transform.Scale(0.8, 2).Move(1.2, -6.8)),
